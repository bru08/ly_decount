...
class H5Dataset(Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        count = (label>0).sum()
        label = label = gaussian_filter(self.labels[index], sigma=(self.g_blur,self.g_blur,self.g_blur), order=0)
        if not (self.horizontal_flip or self.vertical_flip):
            return self.images[index], label, count
        ...
        return (np.flip(self.images[index], axis=axis_to_flip).copy(),
                np.flip(label, axis=axis_to_flip).copy(), count)
